...
class MultiBoxLossLayer : public LossLayer<Dtype> {
 public:
  ...
  // bottom[0] stores the location predictions.
  // bottom[1] stores the confidence predictions.
  // bottom[2] stores the orientation confidence predictions.
  // bottom[3] stores the prior bounding boxes.
  // bottom[4] stores the ground truth bounding boxes.
    
  virtual inline int ExactNumBottomBlobs() const {return 5; } // added by Dong Liu for MTL
 ...
 protected:
  ...
  // The internal orientation confidence loss layer by Dong Liu for MTL
  shared_ptr<Layer<Dtype> > orientation_conf_loss_layer_;
  ConfLossType orientation_conf_loss_type_;
  float orientation_weight_;
  // bottom vector holder used in Forward function.
  vector<Blob<Dtype>*> orientation_conf_bottom_vec_;
  // top vector holder used in Forward function.
  vector<Blob<Dtype>*> orientation_conf_top_vec_;
  // blob which stores the confidence prediction.
  Blob<Dtype> orientation_conf_pred_;
  // blob which stores the corresponding ground truth label.
  Blob<Dtype> orientation_conf_gt_;
  // confidence loss.
  Blob<Dtype> orientation_conf_loss_;
  ...
  int orientation_background_label_id_; // Dong Liu for MTL
  ...
  int num_matches_;
  int orientation_num_matches_;

  int num_conf_;
  int orientation_num_conf_;

  vector<map<int, vector<int> > > all_match_indices_;
  vector<vector<int> > all_neg_indices_;

  vector<map<int, vector<int> > > all_orientation_match_indices_;
  vector<vector<int> > all_orientation_neg_indices_;
