...
bool ProjectBBox(const NormalizedBBox& src_bbox, const NormalizedBBox& bbox,
  ...
  proj_bbox->set_age(bbox.age()); // Added by Dong Liu for MTL
...
void GetDetectionResults(const Dtype* det_data, const int num_det,
  ...
  for (int i = 0; i < num_det; ++i) {
    int start_idx = i * 11; //Change 7 into 11 by Dong Liu for MTL
    ...
    NormalizedBBox bbox;
    // [image_id, label, confidence, xmin, ymin, xmax, ymax, age, ascore, gender, gscore]
    ...
    bbox.set_age(det_data[start_idx + 7]); //Added by Dong Liu for MTL
    bbox.set_ascore(det_data[start_idx + 8]); //Added by Dong Liu for MTL
    bbox.set_gender(det_data[start_idx + 9]); //Added by Dong Liu for MTL
    bbox.set_gscore(det_data[start_idx + 10]); //Added by Dong Liu for MTL
    float bbox_size = BBoxSize(bbox);
    bbox.set_size(bbox_size);
...
void GetAgeDetectionResults(const Dtype* det_data, const int num_det,
  ...
  for (int i = 0; i < num_det; ++i) {
    ...
    NormalizedBBox bbox;
    // [image_id, label, confidence, xmin, ymin, xmax, ymax, gender, gscore]
...
//******************************** Below added by Dong Liu for evaluating gender
template <typename Dtype>
void GetGenderDetectionResults(const Dtype* det_data, const int num_det,
  ...
  for (int i = 0; i < num_det; ++i) {
    ...
    int label = det_data[start_idx + 9];
    //CHECK_NE(background_label_id, label)
    //    << "Found background label in the detection results."; commented on March 10th 2017
    NormalizedBBox bbox;
    // [image_id, label, confidence, xmin, ymin, xmax, ymax, gender, gscore]
...
// Explicit initialization.
template void GetGenderDetectionResults(const float* det_data, const int num_det,
      const int background_label_id,
      map<int, map<int, vector<NormalizedBBox> > >* all_detections);
template void GetGenderDetectionResults(const double* det_data, const int num_det,
      const int background_label_id,
      map<int, map<int, vector<NormalizedBBox> > >* all_detections);
...
void VisualizeBBox(const vector<cv::Mat>& images, const Blob<Dtype>* detections,
  ...
  for (int i = 0; i < num_det; ++i) {
    ...
    NormalizedBBox bbox;
    bbox.set_xmin(detections_data[i * 11 + 3] * width);
    bbox.set_ymin(detections_data[i * 11 + 4] * height);
    bbox.set_xmax(detections_data[i * 11 + 5] * width);
    bbox.set_ymax(detections_data[i * 11 + 6] * height);
    bbox.set_score(score);
    // 
    
    const int age = detections_data[i * 11 + 7];
    const float ascore = detections_data[i * 11 + 8];
  ...
  for (int i = 0; i < num_img; ++i) {
    ...
    for (map<int, vector<NormalizedBBox> >::iterator it =
      ...
      for (int j = 0; j < bboxes.size(); ++j) {
       ...
       if(bboxes[j].age()==1)
    	   age = "1-10";
       else if(bboxes[j].age()==2)
    	   age = "11-20";
       else if(bboxes[j].age()==3)
    	   age = "21-30";
       else if(bboxes[j].age()==4)
    	   age = "31-40";
       else if(bboxes[j].age()==5)
    	   age = "41-50";
       else if(bboxes[j].age()==6)
    	   age = "51-60";
       else if(bboxes[j].age()==7)
    	   age = "61-70";
       else if(bboxes[j].age()==8)
    	   age = "71-80";
       else if(bboxes[j].age()==9)
    	   age = "81-90";
       else if(bboxes[j].age()==10)
    	   age = "91-100";


        snprintf(buffer, sizeof(buffer), "%s: %.2f, %s: %.2f, %s: %.2f", label_name.c_str(),
                 bboxes[j].score(), gender.c_str(), bboxes[j].gscore(), age.c_str(), bboxes[j].ascore()); // added by Dong Liu on March 13th 2017
  ...
  for (int i = 0; i < num_img; ++i) {
    ...
    for (map<int, vector<NormalizedBBox> >::iterator it =
      ...
      for (int j = 0; j < bboxes.size(); ++j) {
        ...
        snprintf(buffer, sizeof(buffer), "%s: %.2f, %s: %.2f, %s: %.2f", label_name.c_str(),
                        bboxes[j].score(), gender.c_str(), bboxes[j].gscore(), age.c_str(), bboxes[j].ascore()); // added by Dong Liu on March 13th 2017
        //snprintf(buffer, sizeof(buffer), "%s: %.2f", gender.c_str(), bboxes[j].gscore()); // added by Dong Liu on March 13th 2017 
