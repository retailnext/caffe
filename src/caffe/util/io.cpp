...
bool ReadXMLToAnnotatedDatum(const string& labelfile, const int img_height,
  ...
  BOOST_FOREACH(ptree::value_type &v1, pt.get_child("annotation")) {
    ...
    if (v1.first == "object") {
      ...
      int age = 0; //Added by Dong Liu for MTL
      int gender = 0; //Added by Dong Liu for MTL
      ...
      BOOST_FOREACH(ptree::value_type &v2, object.get_child("")) {
        ...
        if (v2.first == "name") {
        ...
        } else if (v2.first == "age") {
           age = pt2.get_value<int>();
        }
        else if (v2.first == "gender") {
          gender = pt2.get_value<int>();
          ...
          LOG_IF(WARNING, ymin > ymax) << labelfile <<
              " bounding box irregular.";
          LOG_IF(WARNING, abs(age) < 10e-3) << labelfile <<
              "age is 0.";
          ...
          bbox->set_age(age); //Dong Liu for MTL
          bbox->set_gender(gender); //Dong Liu for MTL
        }
      }
    }
  }
