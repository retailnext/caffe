...
void DetectionOutputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK(detection_output_param.has_num_age_classes()) << "Must specify num_age_classes"; //MTL
  num_age_classes_ = detection_output_param.num_age_classes(); //MTL
  ...
  age_background_label_id_ = detection_output_param.age_background_label_id(); //Added by Dong Liu for MTL
...
void DetectionOutputLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  num_priors_ = bottom[4]->height() / 4; //change bottom[2] into bottom[4]
  CHECK_EQ(num_priors_ * num_loc_classes_ * 4, bottom[0]->channels())
      << "Number of priors must match number of location predictions.";
  CHECK_EQ(num_priors_ * num_classes_, bottom[1]->channels())
      << "Number of priors must match number of confidence predictions.";
  //Added by Dong Liu for MTL
  CHECK_EQ(num_priors_ * num_age_classes_, bottom[2]->channels())
        << "Number of priors must match number of age confidence predictions."; //MTL */
...
void DetectionOutputLayer<Dtype>::Forward_cpu(
  ...
  //const Dtype* age_data = bottom[2]->cpu_data(); //Added by Dong Liu for MTL
  const Dtype* gender_data = bottom[3]->cpu_data(); // added by Dong Liu for MTL
  const Dtype* prior_data = bottom[4]->cpu_data(); //change bottom[2] into bottom[4] for MTL
  ...
  for (int i = 0; i < num; ++i) {
    ...
    const map<int, vector<float> >& gender_scores = all_gender_scores[i]; // added by Dong Liu for MTL
    const LabelBBox& decode_bboxes = all_decode_bboxes[i];
    for (map<int, vector<int> >::iterator it = all_indices[i].begin();
      ...
      for (int j = 0; j < indices.size(); ++j) {
        ...
        top_data[count * 9 + 7] = getIndexOfLargestElement(gscores, gscores.size()) + 1;
        top_data[count * 9 + 8] = *(std::max_element(gscores.begin(), gscores.end()));
        ...
        if (need_save_) {
          ...
          int gender = top_data[count * 9 + 7]; //Added By Dong Liu for MTL
          float gscore = top_data[count * 9 + 8]; // Added By Dong Liu for MTL
          ...
          cur_det.put<float>("gscore", gscore);
